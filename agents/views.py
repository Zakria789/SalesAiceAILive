...
# Import new auto-call system
from .auto_call_system import AutoCallCampaignAPIView
from .campaign_schedule_api import CampaignScheduleAPIView, RealTimeAutoScheduler
from .intelligent_scheduling_api import IntelligentSchedulingRulesAPIView, CallOutcomeStatsAPIView, TestIntelligentSchedulingAPIView
...
def create_agent(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            # 🔥 AUTO-SYNC with HumeAI for outbound agents
            if agent.agent_type == 'outbound':
                print(f"🔹 Agent is outbound, starting HumeAI sync...")
                try:
                    from HumeAiTwilio.hume_agent_service import hume_agent_service
                    import logging
                    
                    logger = logging.getLogger(__name__)
                    logger.info(f"🤖 Creating HumeAI agent for: {agent.name}")
                    print(f"🤖 Creating HumeAI agent for: {agent.name}")
                    
                    # 🔥 Create agent in HumeAI with database fields (sales_script, knowledge_base)
                    config_id = hume_agent_service.create_agent(
                        name=agent.name,
                        system_prompt=f"You are {agent.name}, an AI assistant.",
                        voice_name='Ito',  # Default Ito voice
                        language='en',  # Default English
                        agent_obj=agent  # 🔥 Pass agent object to use sales_script & knowledge_base from DB
                    )
                    
                    print(f"🔹 HumeAI create_agent returned: {config_id}")
                    
                    if config_id:
                        agent.hume_config_id = config_id
                        agent.save(update_fields=['hume_config_id'])
                        logger.info(f"✅ HumeAI agent created with config_id: {config_id}")
                        print(f"✅ HumeAI agent created with config_id: {config_id}")
                        logger.info(f"📝 Sales script included: {bool(agent.sales_script_text)}")
                        logger.info(f"📚 Knowledge base included: {bool(agent.business_info)}")
                    else:
                        logger.warning(f"⚠️ Failed to create HumeAI agent for: {agent.name}")
                        print(f"⚠️ Failed to create HumeAI agent - no config_id returned")
                        
                except Exception as hume_error:
                    logger = logging.getLogger(__name__)
                    logger.error(f"❌ HumeAI sync error: {hume_error}")
                    print(f"❌ HumeAI sync error: {hume_error}")
                    import traceback
                    traceback.print_exc()
                    # Continue even if HumeAI sync fails - agent still created locally
            else:
                print(f"🔹 Agent is not outbound ({agent.agent_type}), skipping HumeAI sync")
            ...
            return Response({
                ...
                'hume_synced': bool(agent.hume_config_id) if agent.agent_type == 'outbound' else None
...
def update_agent(request, agent_id):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            # 🔥 AUTO-UPDATE HumeAI config when agent is updated
            if updated_agent.agent_type == 'outbound' and updated_agent.hume_config_id:
                try:
                    from HumeAiTwilio.hume_agent_service import hume_agent_service
                    import logging
                    
                    logger = logging.getLogger(__name__)
                    logger.info(f"🔄 Updating HumeAI config for: {updated_agent.name}")
                    
                    # Re-create config with updated data (HumeAI doesn't allow partial updates for system_prompt)
                    # Delete old config and create new one
                    old_config_id = updated_agent.hume_config_id
                    
                    # Create new config with updated data
                    new_config_id = hume_agent_service.create_agent(
                        name=updated_agent.name,
                        system_prompt=f"You are {updated_agent.name}, an AI assistant.",
                        voice_name='Ito',  # Default Ito voice
                        language='en',  # Default English
                        agent_obj=updated_agent
                    )
                    
                    if new_config_id:
                        # Delete old config
                        hume_agent_service.delete_agent(old_config_id)
                        
                        # Save new config ID
                        updated_agent.hume_config_id = new_config_id
                        updated_agent.save(update_fields=['hume_config_id'])
                        logger.info(f"✅ HumeAI config updated: {old_config_id} → {new_config_id}")
                    else:
                        logger.warning(f"⚠️ Failed to update HumeAI config for: {updated_agent.name}")
                        
                except Exception as hume_error:
                    logger = logging.getLogger(__name__)
                    logger.error(f"❌ HumeAI update error: {hume_error}")
                    # Continue even if HumeAI sync fails
            ...
            return Response({
                ...
                'agent': response_serializer.data,
                'hume_updated': bool(updated_agent.hume_config_id) if updated_agent.agent_type == 'outbound' else None
            }, status=status.HTTP_200_OK)
        else:
            return Response({
                'success': False,
...
def outbound_agents_list(request):
    """
    Get outbound agents with HumeAI validation
    Returns only agents that have valid HumeAI config_id
    ...
    Response format:
    {
        ...
        "count": number,
        "total_outbound": number  // Total outbound agents
        "synced": number          // Agents with HumeAI config
    }
    """
    try:
        ...
        outbound_agents = Agent.objects.filter(
            owner=request.user,
            agent_type__in=['outbound','outbound_api', 'both'],  # Include 'both' type agents
            status='active'  # Only active agents
        ).order_by('name')
        
        total_outbound = outbound_agents.count()
        
        # Filter agents with valid HumeAI config_id
        validated_agents = []
        
        for agent in outbound_agents:
            # Check if agent has HumeAI config_id
            if agent.hume_config_id:
                validated_agents.append({
                    'id': str(agent.id),
                    'name': agent.name,
                    'hume_config_id': agent.hume_config_id,
                    'voice_model': agent.voice_model or 'en-US-female-1',  # ✅ Fixed: Use voice_model instead of voice_name
        ...
        return Response({
            ...
            'stats': {
                ...
                'synced_with_hume': len(validated_agents),
                'not_synced': total_outbound - len(validated_agents)
            }
        }, status=status.HTTP_200_OK)
        ...
        return Response({
            'success': False,
            'error': f'Failed to fetch outbound agents: {str(e)}'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

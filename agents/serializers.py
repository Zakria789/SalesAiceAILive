...
class AgentCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    business_info = serializers.JSONField(required=False, default=dict)
    
    # Sales script text field (explicitly declared to accept from JSON)
    sales_script_text = serializers.CharField(
        required=False,
        allow_blank=True,
        allow_null=True,
    ...
    class Meta:
        ...
        fields = [
            ...
            'auto_answer_enabled', 'sales_script_file', 'sales_script_text', 'website_url',
            'business_info', 'knowledge_files_upload', 'contacts_file', 'campaign_schedule'
        ]
    ...
    def create(self, validated_data):
        """Create new agent with file handling and HumeAI configuration"""
        ...
        # NOTE: HumeAI sync is handled in views.py create_agent() function
        # Don't create HumeAI config here to avoid conflicts
    ...
    def _create_hume_ai_config(self, agent):
        """Create HumeAI EVI config on HumeAI platform"""
        import requests
        from django.conf import settings
        ...
        try:
            api_key = getattr(settings, 'HUME_AI_API_KEY', '')
            if not api_key:
                print("⚠️  No HumeAI API key found in settings")
                return None
            ...
            sales_script = agent.sales_script_text or "Hello! How can I help you today?"
            knowledge_content = ""
            
            # Get knowledge from BusinessKnowledge entries
            for knowledge in agent.business_knowledge.all():
                if knowledge.knowledge_text:
                    knowledge_content += f"{knowledge.title}:\n{knowledge.knowledge_text}\n\n"
            
            # Map voice model to HumeAI voice name
            voice_mapping = {
                'ITO': 'ITO',
                'KORA': 'KORA', 
                'DACHER': 'DACHER',
                ...
                'WHIMSY': 'WHIMSY',
                'Inspiring Woman': 'ITO',
                'Confident Man': 'DACHER',
                'Professional Female': 'KORA',
                'Professional Male': 'FINN'
            }
            
            voice_name = voice_mapping.get(agent.voice_model, 'ITO')  # Default to ITO
            
            # Create config payload (EVI v3 format)
            config_data = {
                "evi_version": "3",
                "name": f"Voice Agent - {agent.name}",
                "voice": {
                    "provider": "HUME_AI",
                    "name": voice_name
                },
                ...
                "system_prompt": f"""You are {agent.name}, a professional AI voice assistant.
...
TONE: {agent.voice_tone or 'Professional and friendly'}

INSTRUCTIONS:
- Use the opening script when the call starts
- Answer questions using the knowledge base
- Be {agent.voice_tone or 'professional and helpful'}
- Keep responses natural and conversational
""",
                "tools": []
            }
            
            # Create config on HumeAI
            print(f"📤 Creating HumeAI config for: {agent.name}")
            print(f"   API URL: {base_url}/evi/configs")
            ...
            print(f"📥 HumeAI Response Status: {response.status_code}")
            ...
            if response.status_code in [200, 201]:
                ...
                if not config_id:
                    print(f"⚠️  No config_id in response: {result}")
                    return None
                    
                print(f"✅ HumeAI config created successfully: {config_id}")
                return {
                    'success': True,
                    'config_id': config_id,
                    'config_name': f"Voice Agent - {agent.name}"
                }
            elif response.status_code == 409:
                # Config already exists - try to find it
                print(f"⚠️  Config already exists for {agent.name}, searching...")
                existing_config = self._find_existing_hume_config(agent, headers, base_url)
                if existing_config:
                    return existing_config
                return None
            else:
                print(f"❌ HumeAI config creation failed: {response.status_code}")
    ...
    def _find_existing_hume_config(self, agent, headers, base_url):
        """Find existing HumeAI config by agent name"""
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                expected_name = f"Voice Agent - {agent.name}"
                
                # Handle HumeAI response format: {"configs_page": [...]}
                configs = []
                if isinstance(data, dict):
                    configs = data.get('configs_page', data.get('configs', data.get('data', [])))
                elif isinstance(data, list):
                    configs = data
                
                for config in configs:
                    if isinstance(config, dict) and config.get('name') == expected_name:
                        config_id = config.get('id')
                        print(f"✅ Found existing HumeAI config: {config_id}")
                        return {
                            'success': True,
                            'config_id': config_id,
            ...
            print(f"⚠️  Error finding existing config: {e}")
            return None
...
class AgentStatusSerializer(serializers.ModelSerializer):
    ...
    """
    id = serializers.UUIDField(source='pk', read_only=True)  # ✅ Fixed: Use UUIDField instead of IntegerField
